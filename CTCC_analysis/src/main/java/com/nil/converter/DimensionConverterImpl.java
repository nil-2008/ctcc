...
import com.nil.kv.key.ContactDimension;
import com.nil.kv.key.DateDimension;
import com.nil.utils.JdbcInstance;
import com.nil.utils.JdbcUtil;
import com.nil.utils.LRUCache;
 ...
 *
 * <p>做内存缓存，LRUCache 分支 -- 缓存中有数据 -> 直接返回id -- 缓存中无数据 ->
 *
 * <p>查询Mysql 分支：
 *
 * <p>-- Mysql中有该条数据 -> 直接返回id -> 将本次读取到的id缓存到内存中 -- Mysql中没有该数据 -> 插入该条数据 -> 再次反查该数据，得到id并返回缓存到内存中
 *
 * @author lianyou
 * @version 1.0
 */
public class DimensionConverterImpl implements DimensionConverter {
  ...
  public DimensionConverterImpl() {
    // jvm关闭时，释放资源
    Runtime.getRuntime()
        .addShutdownHook(new Thread(() -> JdbcUtil.close(threadLocalConnection.get(), null, null)));
  }
  ...
  private String[] getContactDimensionSQL() {
    String query =
        "SELECT `id` FROM `tb_contacts` WHERE `telephone` = ? AND `name` = ? ORDER BY `id`;";
